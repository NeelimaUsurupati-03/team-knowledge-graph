...
public interface TaskRepository extends Neo4jRepository<Task, String> {
    ...
    @Query("MATCH (t:Task {id: $taskId}), (p:Project {id: $projectId}) MERGE (t)-[:BELONGS_TO]->(p)")
    void assignToProject(@Param("taskId") String taskId, @Param("projectId") String projectId);
    
    @Query("MATCH (t1:Task {id: $taskId}), (t2:Task {id: $dependsOnTaskId}) MERGE (t1)-[:DEPENDS_ON]->(t2)")
    void addDependency(@Param("taskId") String taskId, @Param("dependsOnTaskId") String dependsOnTaskId);
    ...
    @Query("MATCH (t:Task {id: $taskId}), (s:Skill {name: $skillName}) MERGE (t)-[:REQUIRES]->(s)")
    void addSkillToTask(@Param("taskId") String taskId, @Param("skillName") String skillName);
}