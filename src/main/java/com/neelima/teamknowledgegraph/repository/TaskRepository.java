...
public interface TaskRepository extends Neo4jRepository<Task, String> {
    ...
    Task createTask(@Param("id") String id, 
                    @Param("title") String title, 
                    @Param("status") String status, 
                    @Param("priority") String priority);
    
    @Query("MATCH (t:Task {id: $taskId}), (p:Project {id: $projectId}) " +
    	       "MERGE (t)-[:BELONGS_TO]->(p) " +
    	       "RETURN count(*)")
    	void assignToProject(@Param("taskId") String taskId, @Param("projectId") String projectId);
    
    @Query("MATCH (t1:Task {id: $taskId}), (t2:Task {id: $dependsOnTaskId}) " +
    	       "MERGE (t1)-[:DEPENDS_ON]->(t2) " +
    	       "RETURN count(*)")
    	void addDependency(@Param("taskId") String taskId, @Param("dependsOnTaskId") String dependsOnTaskId);
    
    @Query("MATCH (t:Task {id: $taskId})-[:BELONGS_TO]->(proj:Project)<-[:WORKS_ON]-(p:Person)-[:HAS_SKILL]->(s:Skill) " +
    ...
    @Query("MATCH (t:Task {id: $taskId})-[:DEPENDS_ON*1..5]->(blocker:Task) " +
